 ...
 * and eight colors came out white — invisible circles. So the values live here as
 * data, `runtime.ts` writes the frame's variables from them, and anything out
 * here that has to show a series color reads the same array.
 *
 * The order is the color-vision safety mechanism, not cosmetics: each mode's
 ...
 * an automatic flip of dark. s1–s3 are the brand colors charts already used;
 ...
 */
export const SERIES_PALETTE: Record<
  "dark" | "light",
  string[]
> = {
  dark: [
...
/** `amount` of the way from one color to another. */
 ...
 *
 * The picker is a grid because eight colors is a set and thirty-two is a
 ...
 * unrelated color somebody has to judge against the others. Mixed towards
