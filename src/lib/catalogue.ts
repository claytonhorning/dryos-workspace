...
export const catalogue: Dataset[] = [
  ...
  },
  {
    ...
    sourceBasis: "iso_public",
    sourceNotes: [
      "Each run is a zip wrapping one CSV, listed through a JSON endpoint that holds several days of history — which is what lets a missed run catch up on its own instead of leaving a hole.",
      "Every listing entry has Extension \"zip\"; the CSV/XML split is in the friendly name. Filtering on the extension returns both and silently doubles every row.",
      "Prices are published per settlement point with no component breakdown, so the energy, congestion and loss columns are legitimately null here rather than missing.",
      "SCED timestamps are Central local time with a separate repeated-hour flag for the autumn DST fold. Parsed naively, one hour a year is silently overwritten.",
      "The price column arrives space-padded, and negatives appear as \"-.04\" rather than \"-0.04\".",
    ],
    cadence: "5min",
    slaMinutes: 20,
    tier: "realtime",
    availableTiers: ["archive", "standard", "realtime"],
    schema: LMP_SCHEMA,
    primaryKey: PRIMARY_KEY,
    status: "pending",
    telemetry: NO_TELEMETRY,
