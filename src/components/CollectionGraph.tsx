...
export function CollectionGraph({ preview }: { preview: DatasetPreview }) {
  ...
  /*
    ...
    colours, and can only say one flat line.
  ...
  */
  const track = (e: React.MouseEvent) => {
    ...
    setHover((prev) => {
      ...
      return { t, x: r.left + r.width / 2, top: r.top, bottom: r.bottom };
    });
  };

  const hc = hover ? cells.find((c) => c.t === hover.t) : null;
      ...
      >
        {cells.map((c) => (
          <span
            key={c.t}
            data-cell={c.t}
            className={cx("h-3.5 w-3.5 cursor-crosshair rounded-[3px]", CELL_CLASS[c.state])}
            // A ring, not a border: a border would resize the cell under the
            // pointer and walk the whole grid out from under it.
            style={hover?.t === c.t ? { boxShadow: "0 0 0 1.5px var(--color-ink)" } : undefined}
          />
        ))}
      </div>
      {hover && hc && (
        ...
        >
          <div className="font-mono text-[10px] tracking-[0.08em] whitespace-nowrap text-faint">
            {clock(hc.t)}
            {cadence ? ` – ${clock(hc.t + cadence * 1000)}` : ""} {DISPLAY_TZ_LABEL}
          </div>
          <div
            className={cx(
              ...
              hc.state === "onTime" && "text-emerald-400 dr-when-light:text-emerald-700",
              hc.state === "late" && "text-amber-400 dr-when-light:text-amber-600",
              hc.state === "missing" && "text-red-400 dr-when-light:text-red-600",
            )}
          >
            {hc.state === "onTime" ? "on time" : hc.state === "late" ? "late" : "missing"}
          </div>
